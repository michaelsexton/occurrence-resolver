...
class OccurrenceResolver:
        ...
        for occurrence in occurrences:
            ...
            if 'er.mineraloccurrence' in name:
                ...
                if names is not None and len(names) > 0:
                    if self.state == 'vic':
                        name = None
                        logger.log("Victoria has project names for occurrences, setting name to null to protect original", logger.WARNING)
                    else:
                        name = " - ".join(names)
    ...
    def check_representative_commodity(self, occurrence):
        ...
        if commodity_node is None:
            logger.log("No representative commodity found for {0}".format(self.get_id(occurrence.attrib), logger.WARNING))
            return
        commodity_uri = commodity_node.text
        if commodity_uri not in self.commodity_terms:
            logger.log("Commodity {0} not found in lookup for occurrence {1}".format(commodity_uri, self.get_id(occurrence.attrib), logger.WARNING))
    ...
    def process_commodities(self, commodity_nodes):
        ...
        for node in commodity_nodes:
            ...
            if commod is not None:
                commodity = commod.attrib['{http://www.w3.org/1999/xlink}href']
            elif commodname is not None:
                commodity = commodname.text.replace('urn:cgi:classifier:GA:commodity:','')
